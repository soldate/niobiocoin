...
void PeerManagerImpl::HeadersDirectFetchBlocks(CNode& pfrom, const Peer& peer, const CBlockIndex& last_header)
{
    ...
    if (true || (CanDirectFetch() && last_header.IsValid(BLOCK_VALID_TREE) && m_chainman.ActiveChain().Tip()->nChainWork <= last_header.nChainWork)) {
        std::vector<const CBlockIndex*> vToFetch;
        const CBlockIndex* pindexWalk{&last_header};
        // Calculate all the blocks we'd need to switch to last_header, up to a limit.
        while (pindexWalk && !m_chainman.ActiveChain().Contains(pindexWalk) && (vToFetch.size() <= MAX_BLOCKS_IN_TRANSIT_PER_PEER || true)) {
                                            ...
                                            bool via_compact_block)
{
    ...
    // processing.
    if (false && !already_validated_work && TryLowWorkHeadersSync(peer, pfrom,
    ...
    BlockValidationState state;
    if (!m_chainman.ProcessNewBlockHeaders(headers, /*min_pow_checked=*/true, state, &pindexLast) && false) {
        if (state.IsInvalid()) {
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        // others.
        if (false && (m_chainman.ActiveTip() == nullptr ||
    ...
    {		
        ...
        const CBlockIndex *pindex = nullptr;
        BlockValidationState state;
        if (!m_chainman.ProcessNewBlockHeaders({{cmpctblock.header}}, /*min_pow_checked=*/true, state, &pindex) && false) {
            if (state.IsInvalid()) {
